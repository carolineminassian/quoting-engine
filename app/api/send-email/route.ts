...
export async function POST(req: Request) {
  try {
    const { email, clientName, estimateUrl, businessName, userEmail } =
      await req.json();

    // Log for debugging - check your VS Code terminal!
    ...
    const data = await resend.emails.send({
      ...
      html: `
        <div style="font-family: sans-serif; max-width: 600px; margin: 0 auto; border: 1px solid #e5e7eb; border-radius: 8px; padding: 40px; color: #111827;">
          ...
          <a href="${estimateUrl}" style="display: inline-block; background: #2563eb; color: white; padding: 12px 24px; border-radius: 8px; text-decoration: none; font-weight: bold;">
            View Estimate
          </a>
          </div>
